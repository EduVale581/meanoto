...
async function modificarAsistentes(idEvento, idAsistente) {
  try {
    const token = window.localStorage.getItem('token');

    const resp = await fetch(`${API}/modificarAsistentes`, {
      method: 'POST',
      headers: {
        "Content-Type": "application/json",
        'Authorization': 'Bearer ' + token
      },
      body: JSON.stringify({ idEvento: idEvento, idAsistente: idAsistente})
    })
...
async function eliminarAsistente(idEvento, idAsistente) {
  try {
    const token = window.localStorage.getItem('token');

    const resp = await fetch(`${API}/eliminarAsistente`, {
      method: 'POST',
      headers: {
        "Content-Type": "application/json",
        'Authorization': 'Bearer ' + token
